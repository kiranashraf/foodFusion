package com.foodfusion.foodfusion.ImageCache;
...
import com.foodfusion.foodfusion.BuildConfig;
import com.foodfusion.foodfusion.Custom.CircleTransform;
import com.foodfusion.foodfusion.Listeners.ImageFetcherListener;
...
public abstract class ImageWorker {
     ...
     * been added using
     * {@link ImageWorker#addImageCache(Activity, ImageCache.ImageCacheParams)}
     ...
     */
    public void loadDashboardImage(Object data, ImageView imageView) {
        if (data == null) {
            return;
        }
        ...
        if (value != null) {
            ...
            final BitmapCircleWorkerTask task = new BitmapCircleWorkerTask(data, imageView);
            final AsyncCircleDrawable asyncDrawable = new AsyncCircleDrawable(mResources,
                    mLoadingBitmap, task);
    ...
    public void cacheBitmapImage(Object data, ImageFetcherListener imageFetcherListener) {
        if (data == null) {
            return;
        }
        ...
        if (value != null) {
            ...
            // BEGIN_INCLUDE(execute_background_task)
            final BitmapWorkerTask task = new BitmapWorkerTask(data, imageFetcherListener);
            final AsyncDrawable asyncDrawable = new AsyncDrawable(mResources,
                    mLoadingBitmap, task);
    ...
    public void loadBitmapDrawable(Object data, SubsamplingScaleImageView SubsamplingScaleImageView) {
        if (data == null) {
            return;
        }
        ...
        if (value != null) {
            // Bitmap found in memory cache
            SubsamplingScaleImageView.setImage(ImageSource.bitmap(value.getBitmap()));
        } else {//if (cancelPotentialWork(data, SubsamplingScaleImageView)) {
            // BEGIN_INCLUDE(execute_background_task)
            final BitmapWorkerTask task = new BitmapWorkerTask(data, SubsamplingScaleImageView);
            final AsyncDrawable asyncDrawable = new AsyncDrawable(mResources,
                    mLoadingBitmap, task);
            //SubsamplingScaleImageView.setImage(ImageSource.bitmap(asyncDrawable.getBitmap()));
    ...
    public void addImageCache(Activity activity,
        ...
        mImageCache = ImageCache.getInstance(activity, mImageCacheParams);
     ...
     * this imageView. null if there is no such task.
     */
    private static BitmapCircleWorkerTask getBitmapCircleWorkerTask(ImageView imageView) {
        if (imageView != null) {
            final Drawable drawable = imageView.getDrawable();
            if (drawable instanceof AsyncDrawable) {
                final AsyncCircleDrawable asyncDrawable = (AsyncCircleDrawable) drawable;
                return asyncDrawable.getBitmapCircleWorkerTask();
            }
        }
        return null;
    }
    ...
    private class BitmapWorkerTask extends
        ...
        public BitmapWorkerTask(Object data, ImageFetcherListener imageFetcherListener) {
            mData = data;
            this.imageFetcherListener = imageFetcherListener;
        }

        public BitmapWorkerTask(Object data, SubsamplingScaleImageView subsamplingScaleImageView) {
            mData = data;
            subsamplingScaleImageViewWeakReference = new WeakReference<SubsamplingScaleImageView>(subsamplingScaleImageView);
        }
        ...
        protected BitmapDrawable doInBackground(Void... params) {
            ...
            if (mImageCache != null && !isCancelled()
                    && (getAttachedImageView() != null || subsamplingScaleImageViewWeakReference != null || imageFetcherListener != null) && !mExitTasksEarly) {
        ...
        protected void onPostExecute(BitmapDrawable value) {
            ...
            } else if (subsamplingScaleImageViewWeakReference != null) {
                try {
                    SubsamplingScaleImageView subsamplingScaleImageView = subsamplingScaleImageViewWeakReference.get();
                    if (value != null) {
                        subsamplingScaleImageView.setImage(ImageSource.bitmap(value.getBitmap()));
                    }
                } catch (Exception e) {
            ...
            } else if (imageFetcherListener != null) {
                if (value != null) {
                    imageFetcherListener.onImageLoaded();
                } else {
                    imageFetcherListener.onInteruppted();
                }
            }
     ...
     * The actual AsyncTask that will asynchronously process the image.
     */
    private class BitmapCircleWorkerTask extends
            AsyncTask<Void, Void, BitmapDrawable> {
        private Object mData;
        private ImageFetcherListener imageFetcherListener;
        private WeakReference<ImageView> imageViewReference = null;
        ...
        public BitmapCircleWorkerTask(Object data, ImageView imageView) {
            mData = data;
            imageViewReference = new WeakReference<ImageView>(imageView);
        }

        public BitmapCircleWorkerTask(Object data, ImageFetcherListener imageFetcherListener) {
            mData = data;
            this.imageFetcherListener = imageFetcherListener;
        }

        public BitmapCircleWorkerTask(Object data, SubsamplingScaleImageView subsamplingScaleImageView) {
            mData = data;
            subsamplingScaleImageViewWeakReference = new WeakReference<SubsamplingScaleImageView>(subsamplingScaleImageView);
        }
        ...
        protected BitmapDrawable doInBackground(Void... params) {
            ...
            if (bitmap == null && !isCancelled()
                    && (getAttachedImageView() != null || subsamplingScaleImageViewWeakReference != null || imageFetcherListener != null) && !mExitTasksEarly) {
                bitmap = processBitmap(mData);
                CircleTransform cc=new CircleTransform();
                bitmap=cc.transform(bitmap);
            }
        ...
        private ImageView getAttachedImageView() {
            if (imageViewReference != null) {
                final ImageView imageView = imageViewReference.get();
                final BitmapCircleWorkerTask bitmapWorkerTask = getBitmapCircleWorkerTask(imageView);
     ...
     * finish order.
     */
    private static class AsyncCircleDrawable extends BitmapDrawable {
        private final WeakReference<BitmapCircleWorkerTask> bitmapWorkerTaskReference;

        public AsyncCircleDrawable(Resources res, Bitmap bitmap,
                             BitmapCircleWorkerTask bitmapWorkerTask) {
            super(res, bitmap);
            bitmapWorkerTaskReference = new WeakReference<BitmapCircleWorkerTask>(
                    bitmapWorkerTask);
        }

        public BitmapCircleWorkerTask getBitmapCircleWorkerTask() {
            return bitmapWorkerTaskReference.get();
        }
    }
