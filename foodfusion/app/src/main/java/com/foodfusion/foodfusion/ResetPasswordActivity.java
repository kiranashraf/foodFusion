...
public class ResetPasswordActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mOldPass=(EditText)findViewById(R.id.etOldPass);
    ...
    public void BtnResetClick(View v){
        if(checkIfNet()) {
            if (mOldPass.getText().toString().length() > 0 && mNewPass.getText().toString().length() > 0 &&
                ...
                if (mConfirmPass.getText().toString().equals(mNewPass.getText().toString())) {
                    try {
                        ...
                        DataCenter.PasswordChange(this, "password_change", mOldPass.getText().toString(), mNewPass.getText().toString());
    ...
    protected void onResume() {
        ...
        super.onResume();
        this.registerReceiver(mMessageReceiver, new IntentFilter("PasswordChange"));
    ...
    private BroadcastReceiver mMessageReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if(intent.getAction().equals("PasswordChange")){
                SharedPreferences appSharedPrefs = PreferenceManager.getDefaultSharedPreferences(context);
                Gson gson = new Gson();
                String json = "";
                if (appSharedPrefs.contains("PasswordChangeData")) {
                    json = appSharedPrefs.getString("PasswordChangeData", "");
